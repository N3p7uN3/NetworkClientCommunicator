...
public class NetworkCommunicator {
	...
	public NetworkCommunicator()
	{
		...
		_lstMaster = new NetworkComListener() {
			...
			@Override
			public void fireEvent(com.n3p7un3.networkcommunicatorclient.NetworkEvent ne) {
...
//		}
	...
	private void NetworkEvent(NetworkEvent ne)
	{
		if (ne.EventType == NetworkEventType.Connected)
		{
			...
			Log.w("com.n3p7un3.networkcommunicatorclient", "started packet sender thread");
		...
		{
			//Packet is here!
			Log.w("com.n3p7un3.networkcommunicatorclient", "packet received: " + ne.Data);
...
//			}
	...
	private class ClientPacketSender implements Runnable
	{
		...
		public ClientPacketSender(Socket theClient/*, Queue<String> theQueue*/)
		{
			...
			try {
				...
				// TODO Auto-generated catch block
				e.printStackTrace();
				Log.w("com.n3p7un3.networkcommunicatorclient", "Could not start output stream");
		...
		public void run()
		{
				...
				while (!_packetQueue.isEmpty())
				{
					...
					try {
						...
						if ((poll != null) && (!poll.equals("")))
						{
							...
							Log.w("com.n3p7un3.networkcommunicatorclient", "attempting to send: " + packet);
							
							_out.print(packet);
							//_out.flush();
							
							//Log.w("com.n3p7un3.networkcommunicatorclient", "got past output");
							if (_out.checkError())
							{
								Log.w("com.n3p7un3.networkcommunicatorclient", "WARNING: _out error");
								//Thread.currentThread().interrupt();
			...
			//Done
			Log.w("com.n3p7un3.networkcommunicatorclient", "ClientPacketSender stopped");
			_out.close();
		}
	...
	private class ClientPacketReader implements Runnable
	{
		...
		public void run() {
			...
			try {
				_br.close();
				Log.w("com.n3p7un3.networkcommunicatorclient", "ClientPacketReader closed.");
